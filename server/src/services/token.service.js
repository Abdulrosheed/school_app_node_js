 ...
 * @returns {string}
 */
const generateToken = (userId, role, expires, type, secret = "thisisasamplesecret") => {
  const payload = {
    sub: userId,
    role: role,
    iat: moment().unix(),
 ...
 */
const saveToken = async (token, role , userId, expires, type, blacklisted = false) => {
...
const generateAuthTokens = async (user) => {
  ...
  await saveToken(refreshToken, user.role, user.id,refreshTokenExpires, tokenTypes.REFRESH);
...
const generateVerifyEmailToken = async (user) => {
  ...
  await saveToken(verifyEmailToken, user.role , user.id, expires, tokenTypes.VERIFY_EMAIL);
